...
// returns the list of projects by doing a simple query
router.get("/projects", async (req, res) => {
    res.header("Access-Control-Allow-Origin", "*");

    await db.all("SELECT * FROM projects", (error: any, row: any) => {
        ...
        return res.json({ project_list : row });
    });
});
...
router.get("/projects/:id/assignees", async (req, res) => {
    res.header("Access-Control-Allow-Origin", "*");
    ...
    const sql_query =
        ...
        "users.name " +
        "FROM users " +
        "INNER JOIN project_assignees " +
        "ON users.id = project_assignees.user_id " +
...
//     const sql_query = "UPDATE projects " +
//         "SET status = ?" +
//         "WHERE id = ?;"
//
...
router.post("/projects", async (req, res) => {
    ...
    db.run("INSERT INTO projects (name, status)  VALUES ( ?, ?);", [req.body.name, "OPEN"], function(err: any, row:any) {
        console.log("Date/time: " + ts);
        console.log("HTTP method: " + req.method);
        console.log("Path requested: " + "http://localhost:4000/api" + req.path);
        ...
        if(null == err){
            ...
            // @ts-ignore
            return res.json({project_id : this.lastID});
        } else {
